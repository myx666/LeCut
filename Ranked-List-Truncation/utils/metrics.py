...
class Metric:
    ...
    @classmethod
    def nci(cls, labels: np.array, k_s: list, data): # 10*14768/26874
        alpha = {'LeCaRD': 221.1, 'CAIL2021':171.6, 'COLIEE2020':49.0}[data]
        results = []
        for i in range(len(labels)):
            value, x = 0, labels[i]
            for j in range(k_s[i]): 
                value += (1 / math.log(j+2, 2)) if x[j] else (-(j+1)/alpha)
            results.append(value)
        return np.mean(results), results
