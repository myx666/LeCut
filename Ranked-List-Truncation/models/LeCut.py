...
class LeCut(nn.Module):
    def __init__(self, seq_len, input_size, d_model: int=256, n_head: int=4, num_layers: int=1, dropout: float=0.4):
        ...
        self.encoding_layer = nn.LSTM(input_size=6, hidden_size=112 , num_layers=2, batch_first=True, bidirectional=True)
...
if __name__ == '__main__':
    input = t.randn(20, 100, 769)
    # a = input[:,:,:768]
    # b = input[:,:,-1:]
    # print(b.shape)
    # print(t.cat((a, b),2).shape)
    model = LeCut(seq_len=100, input_size = 769)
    result = model(input)
